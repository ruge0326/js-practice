const aux = (i, j, target, array) => {
  // console.log({ i, j, target, array });
  const [a, ...b] = array;
  console.log({ i, j, target, length: b.length });
  if (typeof b !== "object") return [-1, -1];
  j += 1;
  const targetIndex = b.findIndex((number) => {
    return a + number === target;
  });
  // // console.log({ targetIndex, result: targetIndex !== -1 });
  if (targetIndex === -1) {
    return aux(i + 1, j, target, b);
  }
  return [i, j + targetIndex];
};
